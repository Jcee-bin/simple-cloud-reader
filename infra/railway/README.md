...
| `MAX_FILE_BYTES` | `262144000` (250 MiB). |
| `MAX_USER_STORAGE_BYTES` | `2147483648` (2 GiB). |
...
## Small-launch operations

- Start with one API replica. Replace the bounded in-process limiter with
  shared Redis or PostgreSQL storage before using multiple replicas.
- Configure a Railway usage alert and hard spending limit before sharing the
  public URL.
- Watch API memory, CPU, restarts, PostgreSQL connections/storage, bucket
  storage, `429` responses, and readiness failures.
- Run expired-auth cleanup after deployments and at least daily:
...
Enable Railway PostgreSQL backups when the selected plan supports them.
Otherwise, take a daily logical backup from a trusted machine:

```powershell
pg_dump --format=custom $env:DATABASE_URL --file simple-reader.dump
pg_restore --clean --if-exists --no-owner --dbname $env:RESTORE_DATABASE_URL simple-reader.dump
```

Test restoration into a separate PostgreSQL service. Bucket files are
independent of PostgreSQL; retain the bucket and verify several ready records
through signed downloads after restoring.

## Public-beta checklist

1. Confirm `/health/live` and `/health/ready` return `200`.
2. Request and redeem a real email link.
3. Upload, complete, download, and delete one EPUB.
4. Sync one account from two devices and retry one operation ID.
5. Confirm the 121st pull inside one minute returns `429`.
6. Delete a test account and verify its database rows and objects are gone.
7. Confirm logs redact bearer, magic-link, and refresh credentials.
